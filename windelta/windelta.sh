...
do
    ...
    # COMMAND2="SELECT slot,value,substring(block_hash,0,9) as hash,inserted_at,received_at FROM mainnet_builder_block_submission WHERE slot=$SLOT ORDER BY value DESC LIMIT 1;"
    RES=$(psql -p 5433 "host=0.0.0.0 sslmode=disable dbname=defaultdb user=readonly" -t -c "$COMMAND")
    echo $RES
done < winninghashslots2.txt